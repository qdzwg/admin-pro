  ...
  <div class="supplyCList">
    ...
    <Modal v-model="modal"
      ...
      <orderInfo :result="result"
                 searchType="supplyOrder"></orderInfo>
      <Spin fix
            v-if="spinShow">
        <Icon type="load-c"
              size=18
...
export default {
  data () {
    ...
    return {
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            const verification = {
              title: '核销',
              action: async () => {
                let res = await apiGet('/order/orderInfo/toCheckOrderFL', { id: params.row.ordId })
                if (res.success) {
            ...
            const confirmOrder = {
              ...
              action: () => {
                ...
                this.$refs.confirmModel.confirm(
                  ...
                  { orderNo: params.row.orderNo, busType: "C" }
                )
              }
            }
            // 酒店二次确认取消订单
            ...
            const toReSchedule = {
              title: '改签',
              action: async () => {
                let res = await apiPost('/order/webapi/orderInfo/toReSchedule', { id: params.row.ordId })
                if (res.status === 200) {
            ...
            let refund = {
              title: '退单',
              action: async () => {
                let res = await apiGet(params.row.orderType === "traffic"?'/order/api/toRefundB2BTraffic':'/order/orderInfo/toRefundB2BFL', { id: params.row.ordId })
                if (res.success) {
            ...
            const actions = [
              {
                title: '明细',
                action: async () => {
                  let res = await apiGet('/order/orderInfo/viewOrderSupplyFL', {
                    id: params.row.id
                  })
                  if (res.success) {
                    this.modal = true
            ...
            if (
              ...
              if (params.row['orderType'] === 'mdse') {
                if (
                  ...
                  params.row['groupStatus'] !== '0' &&
                  params.row['groupStatus'] !== '2'
                ) {
      ...
      url: '/order/orderInfo/gridSupply',
      modal: false,
      modal2: false,
      formItem: {
        name: '',
  ...
  async created () {
    let res = await apiPost('/order/webapi/orderInfo/gridSupply')
    if (res.status === 200) {
