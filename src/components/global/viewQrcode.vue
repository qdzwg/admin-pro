...
export default {
  ...
  watch: {
    value: {
      handler (value) {
        this.preLookModal = value
        if (value) {
          this.open(this.url, this.param)
        }
      },
      immediate: true
    },
    // url: {
    //   handler (value) {
    //     this.open(value, this.param)
    //   }
    // },
    // param: {
    //   handler (value) {
    //     this.open(this.url, value)
    //   }
    // }
  },
  methods: {
    open (url, param) {
      this.apiGet(url, param).then(res => {
        if (res.success || res.status === 200) {
          ...
          // 把生成的二维码转换成图片
          this.$nextTick(() => {
            let canvasData = document.getElementsByTagName('canvas')
