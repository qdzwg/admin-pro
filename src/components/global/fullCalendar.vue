  ...
  <div class="calendar-panel">
    ...
    <div class="calendar-contnet">
      <table class="calendar-table">
        ...
        <tr v-for="index in row"
            :key="index">
          <td v-for="item in 7"
              :key="item"
              @click="cellHandle(dayNum[7*(index-1)+(item-1)]._isDisabled,7*(index-1)+(item-1),item)">
            <div v-if="dayNum[7*(index-1)+(item-1)]&&dayNum[7*(index-1)+(item-1)]._isShow"
              ...
              <p class="calendar-explian"
                 v-if="!(dayNum[7*(index-1)+(item-1)]&&dayNum[7*(index-1)+(item-1)]._isDisabled)">
                <span v-for="eitem in extend"
                      :key="eitem.key">
                  {{eitem.title}}：
                  <template v-if="eitem.render">
                    <renderItem :render="eitem.render"
                                :column="dayNum[7*(index-1)+(item-1)]"></renderItem>
                  </template>
                  <template v-else>
...
export default {
  data () {
    return {
      ...
      today: new Date().getFullYear() + '-' + this.p(new Date().getMonth() + 1) + '-' + this.p(new Date().getDate()),
      spinShow: true
    }
  },
  components: {
  ...
  methods: {
    ...
    async getMonthDays (year, month) {
      ...
      if (this.action) {
        ...
        let res = await this.apiPost(this.action, Object.assign(param, this.param))
        // console.log(res)
    ...
    setCheckAll () {
      if (this.sundayCheckAll && this.mondayCheckAll && this.tuesdayCheckAll && this.wednesdayCheckAll && this.thursdayCheckAll && this.fridayCheckAll && this.saturdayCheckAll) {
        this.checkAll = true
      } else {
        this.checkAll = false
      }
