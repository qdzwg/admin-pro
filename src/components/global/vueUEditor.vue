...
export default {
  ...
  methods: {
    _initEditor () {
      ...
      this.editor = window.UE.getEditor(this.id, Object.assign(this.defaultConfig, this.config))
      this.editor.addListener('ready', () => {
        this.status = 2
        // this.$emit('ready', this.editor)
        this._setContent(this.initValue || '')
    ...
    _normalChangeListener () {
      ...
      this.editor.addListener('blur', (event) => {
        this.$emit('input', this.editor.getContent())
        this.dispatch('FormItem', 'on-form-blur', this.editor.getContent())
      })
    },
    _setContent (value) {
