<template>
    ...
    <Row :gutter="16" class="btn-groups">
        ...
        <!-- <Col span="3"><Button type="success" icon="android-done" @click="open">可开票产品</Button></Col> -->
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          sortable: true,
           render:(h,params)=>{
              // return h("span",params.row.showAddress.replace("<br/>"," / "))
        ...
        },
        {
          ...
          sortable: true,
          render:(h,params)=>{
            return h("span",params.row.sumAmount ? params.row.sumAmount.toFixed(2): '' + ' / ' + params.row.taxAmount ? params.row.taxAmount.toFixed(2) : '')
          }
        },
        {
          title: "状态请求状态|返回状态",
        ...
        //       },
        //       {
        //         title: "次数重置"+'('+params.row.taskTime + ')',
        //         action: () => {
        //           this.setModal = true;                  
        //         }
        //       }
  ...
  created() {
    this.apiPost('/marketing/marketElectronicInvoices/listFL').then(res => {
          if(res.success){
              ...
              this.url = '/marketing/marketElectronicInvoices/grid'
          }
      })
  },
  methods: {
