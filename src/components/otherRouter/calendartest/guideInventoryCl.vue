<template>
    <div class="inventoryCl">
        <Row>
            <Col span="18">
            <div class="head-box">
                <h3 class="i-b calendar-title"> {{productName}}</h3>
            </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr>
                            <td>成本价</td>
                            <td>
                              <FormItem prop="price" :rules="[{required: true, message: '该项不能为空！', trigger: 'blur'},{ pattern: /^(([0-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/, message: '数据格式不正确'}]">
                                <Input v-model="formItem.price" :maxlength="6"></Input>
                              </FormItem>
                        ...
                        <tr>
                            ...
                            <td>
                                <FormItem prop="merchantLinePrice"
                                          ...
                                          { pattern: /^(([0-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/, message: '数据格式不正确'}]">
                        ...
                        <tr>
                            <td>零售价</td>
                            <td>
                              <FormItem prop="salePrice" :rules="[
                              {required: true, message: '该项不能为空！', trigger: 'blur'},
                              {pattern: /^(([0-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/, message: '数据格式不正确'}]">
                                <Input v-model="formItem.salePrice" :maxlength="6"></Input>
                              </FormItem>
                        ...
                        <tr>
                            <td>日库存</td>
                            <td>
                                <FormItem prop="totalNum" :rules="[
                                {required: true, message: '该项不能为空！', trigger: 'blur'},
                                { pattern: /^(([0-9]*))$/, message: '数据格式不正确'}]">
...
export default {
  ...
  data () {
    return {
      ...
      salePriceStatus: 'T',
      action: '/product/api/chargeGuideInfo/selectGuideCalendar',
      extend: [
        {
          title: '零售价',
          key: 'salePrice',
  ...
  methods: {
      ...
      this.$refs.formItem.validate(async valid => {
        if (valid) {
          let res = await this.apiPost(
            '/product/api/chargeGuideInfo/saveGuideDailyPriceStock',
            formData
          )
          if (res.status == 200) {
    ...
    getCheckInfo (items) {
      if (items.length) {
        ...
        this.formItem.merchantLinePrice = typeof items[0].merchantLinePrice !== 'undefined' ? items[0].merchantLinePrice + '' : ''
        this.formItem.totalNum = typeof items[0].totalNum !== 'undefined' ? items[0].totalNum + '' : ''
        this.formItem.salePriceStatus = items[0].salePriceStatus
        this.formItem.remark = items[0].remark
        this.formItem.price = typeof items[0].price !== 'undefined' ? items[0].price + '' : ''
        this.formItem.salePrice = typeof items[0].salePrice !== 'undefined' ? items[0].salePrice + '' : ''
      }
    }
  },
  components: {
