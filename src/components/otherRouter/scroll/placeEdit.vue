  ...
  <div class="place-edit-warper">
    ...
    >
      ...
      <FormItem label="场所地址：" prop="address">
        <Input v-model="formItem.address" style="width:33%" :maxlength="64"></Input>
      </FormItem>
      <FormItem label="联系电话：" prop="tel">
        <Input v-model="formItem.tel" style="width:33%" :maxlength="64"></Input>
      </FormItem>
      <FormItem label="场所状态：" prop="state">
        <Select v-model="formItem.state" style="width:200px">
          <Option value="T">上架</Option>
          <Option value="F">下架</Option>
        </Select>
      </FormItem>
      <FormItem label="座位分布图：" prop="imgUrl">
...
export default {
  data() {
    ...
    return {
      ...
      ruleComplteForm: {
        ...
        imgUrl: [{ required: true, message: "请上传座位分布图" }],
        tel: [
          {
  ...
  methods: {
    ...
    //初始化发布页面
    setWapUrl(imgArr) {
      this.formItem.imgUrl = imgArr.join(",");
      this.$refs.formItem.validateField("imgUrl");
    },
    submit() {
