<template>
  <div class="ruleAndCalendar-box">
    <Row>
      ...
      <full-calendar v-if='show' @dateSelect='dateData' url="/merchant/merchantProductPrice/getCalendarHotelInfo" ref="fullCa" class="test-fc" first-day='1' locale="zh-cn" @changeSelectAll="changeValue" @changeMonth="changeMonth" @eventClick="eventClick" @dayClick="dayClick" @moreClick="moreClick">
...
<script>
import fullCalendar from "./../calendartest/priceCalendar";
...
export default {
  ...
  created() {
    apiGet("/merchant/merchantProductPrice/calendarPageHotelInfo", {
      modelCode: this.$route.query.modelCode
    }).then(res => {
      this.routeName = res.data.hotelRoom.nickName;
    });
  },
  computed: {
    ticketTypeArr() {
  ...
  methods: {
    ...
    dateData(social) {
      if(social.length){
        let dateData = this.social[0].dateData;
        this.formValidate = {
