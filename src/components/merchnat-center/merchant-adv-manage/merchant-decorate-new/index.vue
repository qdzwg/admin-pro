      ...
      <TabPane label="自定义模板" name="name2">
        <div style="overflow:auto">
          <div class='index-handel-box'>
            ...
            <Button v-if="status=='0'" type="primary" @click="done">
              <span>启用</span>
            </Button>
          ...
          <span style="float:left;margin-top:43px;font-size:16px:color:#999">扫一扫预览，只针对首页样式预览</span>
        </div>
    ...
    <Modal v-model="confirmModel" width="360" class-name="ivu-confirm-modal">
      ...
      <div>
        <p style="color:red;font-size:14px">
          {{handleState=='done'?'启用后C端将更换为自定义模板，请谨慎操作 ！':'禁用后C端将更换为基础模板，请谨慎操作 '}}
        </p>
      </div>
...
export default {
  ...
  created() {
    this.getData();
    let url = "http://wap.lotsmall.cn?m_id="; //正式
    // let url = "http://testwap.lotsmall.cn/showNewIndex?m_id="; //测试
    apiGet("/merchant/merchantInfo/getUserMerchantInfo").then(res => {
      this.qrcodeUrl = url + res.id;
      this.merchantInfoId=res.id
      this.show = true;
    });
  },
  components: {
