<template>
  <div>
    ...
    <Modal width="800" v-model="buyRuleModal" title="添加购买规则" :loading="loading" @on-ok="ok"  @on-cancel="cancel">
        <Form  class="defaultHeight" :model="formBuyRuleItem" label-position="right" :label-width="120" ref="formItem" :rules="buyRuleForm">
...
export default {
  data () {
    return {
          ...
          render: (h, params) => {
            ...
            },
            {
              ...
              action: () => {
                ...
                this.content = "确认删除吗？"
                this.sucessMsg = "删除成功!"
                this.$refs.confirmModel.confirm('list/product/ruleProduct/del?ids=' + params.row.id + '&' + params.row.id)
              }
            }]
      ...
      url: 'list/product/ruleProduct/grid',
      searchItems: [
        {
          label: '产品规则名称',
          type: 'input',
  ...
  methods: {
    ...
    afterSaleOk(){
      let _this = this;
      let url = "list/product/ruleProduct/save/ruleSold";
